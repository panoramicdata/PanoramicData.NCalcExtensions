...
internal static class Format
{
	internal static void Evaluate(FunctionEventArgs functionArgs, CultureInfo cultureInfo)
	{
		const int min = 2;
		const int max = 3;
		if (functionArgs.Parameters.Count is < min or > max)
		{
		...
		if (functionArgs.Parameters.Evaluate(1) is not string formatFormat)
		{
			throw new ArgumentException($"{ExtensionFunction.Format} function - expected second argument to be a format string");
		}

		functionArgs.Result = (inputObject, functionArgs.Parameters.Count) switch
		{
			...
			(DateTime dateTime, 3) => dateTime.ToDateTimeInTargetTimeZone(formatFormat, functionArgs.Parameters.Evaluate(2) as string ?? throw new ArgumentException($"{ExtensionFunction.Format} function - expected third argument to be a TimeZone string"), cultureInfo),
			(DateTimeOffset dateTimeOffset, 2) => dateTimeOffset.UtcDateTime.BetterToString(formatFormat, cultureInfo),
			(DateTimeOffset dateTimeOffset, 3) => dateTimeOffset.UtcDateTime.ToDateTimeInTargetTimeZone(formatFormat, functionArgs.Parameters.Evaluate(2) as string ?? throw new ArgumentException($"{ExtensionFunction.Format} function - expected third argument to be a TimeZone string"), cultureInfo),
