...
internal static class Min
{
	internal static void Evaluate(FunctionEventArgs functionArgs)
	{
		...
		var originalList = originalListUntyped as IEnumerable
			?? throw new FormatException($"First {ExtensionFunction.Min} parameter must be an IEnumerable.");

		if (functionArgs.Parameters.Count == 1)
		{
			functionArgs.Result = MinOf(originalList);
			return;
		}
		...
		functionArgs.Result = MinOf(originalList, lambda);
	}

	/// <summary>
	/// The minimum of the list's own values.
	/// </summary>
	private static object? MinOf(IEnumerable originalList) => originalList switch
	{
	...
	/// <summary>
	/// The minimum of <paramref name="lambda"/> applied to each of the list's values.
	/// </summary>
	private static object? MinOf(IEnumerable originalList, Lambda lambda) => originalList switch
	{
