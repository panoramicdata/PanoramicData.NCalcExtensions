...
public class AnyTests : NCalcTest
{
	...
	public void Any_EmptyList_ReturnsFalse()
		=> Test("any(list(), 'n', 'n > 0')").Should().Be(false);

	[Fact]
	public void Any_OneMatches_ReturnsTrue()
		=> Test("any(list(1, 3, 5, 6), 'n', 'n % 2 == 0')").Should().Be(true);

	[Fact]
	public void Any_AllMatch_ReturnsTrue()
		=> Test("any(list(2, 4, 6, 8), 'n', 'n % 2 == 0')").Should().Be(true);

	[Fact]
	public void Any_NoneMatch_ReturnsFalse()
		=> Test("any(list(1, 3, 5, 7), 'n', 'n % 2 == 0')").Should().Be(false);

	[Fact]
	public void Any_Strings_Works()
		=> Test("any(list('cat', 'dog', 'bird'), 's', 'startsWith(s, \"d\")')").Should().Be(true);

	[Fact]
	public void Any_Strings_NoneMatch()
		=> Test("any(list('cat', 'rat', 'bat'), 's', 'startsWith(s, \"d\")')").Should().Be(false);

	[Fact]
	public void Any_GreaterThan_Works()
		=> Test("any(list(1, 2, 10), 'n', 'n > 5')").Should().Be(true);

	[Fact]
	public void Any_LessThanOrEqual_Works()
		=> Test("any(list(10, 20, 5), 'n', 'n <= 5')").Should().Be(true);

	[Fact]
	public void Any_ComplexCondition_Works()
		=> Test("any(list(5, 15, 25), 'n', 'n >= 10 && n <= 20')").Should().Be(true);
	...
	public void Any_SingleItemTrue_ReturnsTrue()
		=> Test("any(list(5), 'n', 'n > 0')").Should().Be(true);

	[Fact]
	public void Any_SingleItemFalse_ReturnsFalse()
		=> Test("any(list(-5), 'n', 'n > 0')").Should().Be(false);

	[Fact]
	public void Any_Doubles_Works()
		=> Test("any(list(1.1, 1.2, 2.5), 'n', 'n > 2.0')").Should().Be(true);

	[Fact]
	public void Any_NegativeNumbers_Works()
		=> Test("any(list(10, 20, -5), 'n', 'n < 0')").Should().Be(true);

	[Fact]
	public void Any_WithNulls_HandlesNull()
		=> Test("any(list(1, null, 3), 'n', 'n == null')").Should().Be(true);

	[Fact]
	public void Any_Booleans_True()
		=> Test("any(list(false, false, true))").Should().Be(true);
